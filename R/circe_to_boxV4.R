...
boxr::box_auth(client_id = "f8myfa4diw1bd7bo083eqayw228dp4l7",
               client_secret = "arc8rBKdq4UPQ5pbTiA8QbfnqH17cSoR")
...
box_to_circe <- function(dir_name, working_dir){
  ...
  box.d <- boxr::box_dir_create(dir_name = box_dir, parent_dir_id = "137932543788")
...
get_id <- boxr::box_search_folders(query = query_term, # search for current folder. If exist, get folder id
                                   ancestor_folder_id = "137932543788",
                                   content_types = "name") %>%
  as.data.frame()
get_id <- get_id$id[1]
